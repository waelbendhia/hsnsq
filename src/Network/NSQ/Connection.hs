...
module Network.NSQ.Connection ( defaultConfig, establish,) where
...
import System.Log.Logger (debugM, errorM, infoM, warningM)
...
defaultConfig serverHost = do
  ...
  pure $ NSQConnection serverHost 4150 "NSQ.Connection." localIdent
...
handleReply = forever . (yield <=< liftIO . atomically . readTQueue)
...
handleNSQ ::
  ...
  TQueue Message ->
  m ()
handleNSQ sc recv send topicQueue =
  void $
    runEffect $
      nsqParserErrorLogging (logName sc) recv
        >-> (command (logName sc) topicQueue)
        >-> showCommand
        >-> send
...
nsqParserErrorLogging ::
  MonadIO m =>
  LogName ->
  Producer BS.ByteString m () ->
  Producer NSQ.Message m ()
...
command ::
  ...
  LogName ->
  TQueue Message ->
  Pipe NSQ.Message NSQ.Command m ()
command l topicQueue = forever $ do
  ...
  case msg of
    ...
    NSQ.CatchAllMessage f m ->
      liftIO $ warningM l ("Error: Frame - " ++ show f ++ " - Msg - " ++ show m)
