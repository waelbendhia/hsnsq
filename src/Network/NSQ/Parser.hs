...
module Network.NSQ.Parser (message, decode, encode) where
  ...
  BL.toStrict $
    BL.toLazyByteString $
      BL.byteString "MPUB "
        ...
        <> BL.word32BE totalCount -- Accord for message count
        <> content
  where
  ...
  BL.toStrict $
    BL.toLazyByteString $
      BL.byteString "RDY "
        ...
        <> BL.byteString "\n"
encode (Fin msg_id) =
  BL.toStrict $ BL.toLazyByteString 
    $ BL.byteString "FIN " <> BL.byteString msg_id <> BL.byteString "\n"
encode (Touch msg_id) =
  BL.toStrict $
    BL.toLazyByteString $
      BL.byteString "TOUCH " <> BL.byteString msg_id <> BL.byteString "\n"
encode (Req msg_id timeout) =
  BL.toStrict $
    BL.toLazyByteString $
      BL.byteString "REQ "
